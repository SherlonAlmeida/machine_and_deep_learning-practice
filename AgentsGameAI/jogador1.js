...
jogador1.ia = function (tempo) {
	...
	if(this.meuTamanho() > this.tamanhoDoInimigo()){
		...
		if((this.distanciaDoInimigo()) < 100 && (this.tamanhoDoInimigo() > this.meuTamanho())){
			inim = Math.abs(this.anguloDoInimigo() - 360);
			this.direcionar(inim);
			this.acionarToxina();
			if((this.distanciaAteUmaParede("superior") <= 5) || (this.distanciaAteUmaParede("inferior") <= 5) || (this.distanciaAteUmaParede("esquerda") <= 5) || (this.distanciaAteUmaParede("direita")  <= 5)){
				angulo = Math.abs(this.anguloDaParedeMaisProxima() - 360);
				this.direcionar(angulo+90);
			}
		}
		else{
